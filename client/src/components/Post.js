...
export default function Post({ loggedIn, setDeletePost }) {
  ...
  useEffect(() => {
    (async () => {
      let postResponse = await axios.get(`https://catit.herokuapp.com/posts/${slug}`);
  ...
  async function handleCommentDelete(id) {
    try {
      let deleteResponse = await axios.delete(
        `https://catit.herokuapp.com/comments/${id}`,
        {
          headers: {
  ...
  async function makeComment() {
    try {
      const response = await axios.post(
        `https://catit.herokuapp.com/comments/${slug}`,
        {
          content: commentArea,
        },
        {
  ...
  async function handleDelete() {
    try {
      let deleteResponse = await axios.delete(
        `https://catit.herokuapp.com/posts/${slug}`,
        {
          headers: {
            authorization: `Bearer ${cookies.get("accessToken")}`,
          },
